...
import {
  ...
  removeSizeMaster,
  updateSizeMaster,
} from "../../functions/Category/SizeCategory";
...
const SizeCategory = () => {
  ...
  const { sizeType, Category, IsActive } = values;
  ...
  const validate = (values) => {
    ...
    if (values.sizeType === "") {
      errors.sizeType = "size Type is required!";
      setErrTP(true);
    }
    if (values.sizeType !== "") {
      setErrTP(false);
  ...
  const fetchCategories = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_COFFEE}/api/auth/list-by-params/size-category`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
        ...
        <form>
          <ModalBody>
            ...
            <div className="form-floating mb-3">
              ...
              />
              <Label>
                size Type <span className="text-danger">*</span>
              </Label>
              {isSubmit && <p className="text-danger">{formErrors.sizeType}</p>}
            </div>
