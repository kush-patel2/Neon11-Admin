...
import {
  ...
  removePartner,
  listPartner,
} from "../../../functions/PartnerLogin/PartnerLogin";
...
const PartnerLogin = () => {
  ...
  const { ContactName, UserName, Password, IsActive } = values;
  ...
  const loadkarats = () => {
    listPartner().then((res) => setKarats(res));
  };
  ...
  const fetchCategories = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_ZIYA}/api/auth/partners/ziya-by-params`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  const validClassUName =
    errUName && isSubmit ? "form-control is-invalid" : "form-control";

  const validClassCName =
    errCName && isSubmit ? "form-control is-invalid" : "form-control";
  ...
  document.title = "Partner Details | ZIYA";
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        >
          <span style={{ marginRight: "210px" }}>Remove Partner Details</span>
          {/* <Button
            type="button"
            onClick={() => {
