...
} from "../../functions/ProductParameters/ParameterMaster";
...
const ProductParameterMaster = () => {
  const [values, setValues] = useState(initialState);
  const { parameterName, IsActive } = values;
  ...
  const handleDelete = (e) => {
    e.preventDefault();
    removeParameterMaster(remove_id)
      .then((res) => {
        setmodal_delete(!modal_delete);
        fetchCategories();
      })
  ...
  const handleUpdate = (e) => {
    ...
    if (Object.keys(erros).length === 0) {
      updateParameterMaster(_id, values)
        .then((res) => {
          setmodal_edit(!modal_edit);
          fetchCategories();
        })
  ...
  const fetchCategories = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_COFFEE}/api/auth/list-by-params/parameterMaster`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  document.title = "Product Parameter | RC Henning Coffee Company";
        ...
        <form>
          <ModalBody>
            <div className="form-floating mb-3">
              ...
              {isSubmit && (
                <p className="text-danger">{formErrors.parameterName}</p>
              )}
            </div>
...
export default ProductParameterMaster;
